...
// various std::string utilities built upon the 'str' module
...
// direct std::string replacement, replacing instances of oldstr with newstr
// (newstr may be "")
std::string replace(rostring src, rostring oldstr, rostring newstr);

// works like unix "tr": the source std::string is translated character-by-character,
...
std::string translate(rostring src, rostring srcchars, rostring destchars);
...
// remove any whitespace at the beginning or end of the std::string
std::string trimWhitespace(rostring str);
// dsw: get the first alphanum token in the std::string
std::string firstAlphanumToken(rostring str);


// encode a block of bytes as a std::string with C backslash escape
...
std::string encodeWithEscapes(rostring src);

// adds the quotes too
std::string quoted(rostring src);


// decode an escaped std::string; throw xFormat if there is a problem
...
// given a std::string with quotes and escapes, yield just the std::string;
...
// renamed from 'basename' because of conflict with something in std::string.h
std::string sm_basename(rostring src);

// given a directory name like "a/b/c", return "a/b"; if 'src' contains
// no slashes at all, return "."
std::string dirname(rostring src);
...
// Sometimes it's useful to store a std::string value in a static buffer;
